...
% Awesome CV LaTeX Template for CV/Resume (adapted for awesomeresume)
%
% Adapted to be an Rmarkdown template by Zhenguo Zhang
% Modified for awesomeresume: tighter margins and monochrome output
%
%-------------------------------------------------------------------------------
% CONFIGURATIONS
%-------------------------------------------------------------------------------
% A4 paper size by default, use 'letterpaper' for US letter
\documentclass[$if(fontsize)$$fontsize$$else$11pt$endif$,$if(lang)$$babel-lang$,$endif$$if(papersize)$$papersize$$else$a4paper$endif$,$for(classoption)$$classoption$$sep$,$endfor$]{awesome-resume}

% Configure page margins with geometry (awesomeresume defaults)
$if(geometry)$
\usepackage{geometry}
...
\renewcommand{\acvHeaderAfterSocialSkip}{0.5mm}
\renewcommand{\acvHeaderAfterQuoteSkip}{1mm}

% Allow YAML variable `section_spacing` (LaTeX length, e.g. '-1mm' or '0.5mm')
$if(section_spacing)$
\makeatletter
\let\origsection\section
\renewcommand{\section}[1]{\origsection{#1}\vspace{$section_spacing$}}
\makeatother
$endif$
...
\renewcommand*{\entrydatestyle}[1]{{\fontsize{8pt}{1em}\bodyfont\slshape\color{graytext} #1}}
...
%  \vspace{-2.0mm}
  ...
  \end{tabular*}%
  \vspace{-2.0mm} % adjust this value to change spacing after each entry
}
